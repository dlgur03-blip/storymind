...
import {
  X, ArrowRight, ArrowLeft, BookOpen, Wand2, Shield,
  PenLine, Keyboard, Sparkles, FolderOpen, Maximize2, Upload
} from 'lucide-react'
...
const STEPS = [
  {
    ...
    description:
      '소설 작가를 위한 AI 집필 도우미입니다. 설정 오류 검수, 복선 관리, AI 대필까지 - 창작에만 집중하세요.',
    tip: '이 가이드는 언제든 다시 볼 수 있어요.',
  },
  {
    icon: Upload,
    title: '기존 원고 임포트',
    description:
      '대시보드에서 "임포트" 버튼을 누르면 .txt 파일들을 한번에 업로드할 수 있습니다. 각 파일이 하나의 챕터로 변환됩니다.',
    features: [
      { label: '.txt 파일', desc: '텍스트 파일을 챕터로 변환' },
      { label: '일괄 업로드', desc: '여러 파일 동시 선택 가능' },
    ],
  },
  {
    icon: PenLine,
    title: '에디터에서 집필',
    description:
      '화(챕터) 단위로 집필합니다. 자동 저장되며, 좌측에서 챕터를 관리하고 우측에서 AI 기능을 사용합니다.',
    features: [
  ...
  },
  {
    ...
    title: 'AI 검수 (핵심 기능)',
    description:
      '에디터 우측 패널에서 AI 검수를 실행하면 여러 모듈이 동시에 분석합니다.',
    features: [
  ...
  },
  {
    ...
    description:
      '우측 패널에서 캐릭터, 복선, 세계관, 시간선을 관리합니다. AI 검수 시 자동으로 업데이트됩니다.',
    features: [
  ...
  },
  {
    ...
    title: 'AI 대필 & 제안',
    description:
      '아이디어만 입력하면 AI가 현재 설정과 문체에 맞게 글을 작성합니다.',
    features: [
...
export default function OnboardingGuide({ onClose }: OnboardingGuideProps) {
  ...
  return (
    ...
    >
      <div
        className={`w-full max-w-lg mx-4 bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl overflow-hidden transition-transform duration-200 ${
      ...
      >
        ...
        {/* Content */}
        <div className="px-6 pb-6" key={step}>
          ...
          {current.features && (
            <div className="grid grid-cols-2 gap-2 mb-4">
              {current.features.map((f, i) => (
                <div
                  key={i}
                  className="p-2.5 bg-neutral-50 dark:bg-neutral-800 rounded-lg"
                >
          ...
          {current.shortcuts && (
            <div className="space-y-1.5 mb-4 max-h-48 overflow-y-auto">
              {current.shortcuts.map((s, i) => (
                ...
                >
                  <kbd className="px-2 py-0.5 bg-white dark:bg-neutral-700 border border-neutral-200 dark:border-neutral-600 rounded text-xs font-mono text-neutral-700 dark:text-neutral-300">
                    {s.key}
                  </kbd>
        ...
        <div className="flex items-center justify-between px-6 pb-6">
          ...
          <div className="flex items-center gap-3">
            {step < STEPS.length - 1 && (
              <button
                onClick={close}
                className="text-sm text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300 transition"
              >
                건너뛰기
              </button>
            )}
...
export function useOnboarding() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && !localStorage.getItem('sm_onboarding_done')) {
      setShow(true)
    }
  }, [])
