...
// 데스크톱 앱: 기본 유저 생성 (ID=1)
const defaultUser = db.prepare('SELECT id FROM users WHERE id=1').get();
if (!defaultUser) {
  db.prepare("INSERT INTO users (id, username, email, password_hash, nickname) VALUES (1, 'local', 'local@desktop', 'desktop', '작가')").run();
